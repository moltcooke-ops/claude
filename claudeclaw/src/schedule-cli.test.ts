...
describe('schedule-cli agent routing', () => {
  ...
  it('auto-detects agent from CLAUDECLAW_AGENT_ID env var', () => {
    const result = createAndTrack(
      ...
      { ...process.env, CLAUDECLAW_AGENT_ID: 'comms', DB_ENCRYPTION_KEY: TEST_ENCRYPTION_KEY },
    );
  ...
  it('defaults to main when no env var and no --agent flag', () => {
    const result = createAndTrack(
      ...
      { ...process.env, CLAUDECLAW_AGENT_ID: undefined, DB_ENCRYPTION_KEY: TEST_ENCRYPTION_KEY },
    );
  ...
  afterEach(() => {
    ...
    for (const id of createdTaskIds) {
      try {
        execSync(`node "${CLI_PATH}" delete ${id}`, { cwd: PROJECT_DIR, env: { ...process.env, DB_ENCRYPTION_KEY: TEST_ENCRYPTION_KEY } });
      } catch {
